...
class CustomKlineDataset(Dataset):
    ...
    def _split_data_by_time(self):
        ...
        if self.data_type == "train":
            ...
            print(
                f"[{self.data_type.upper()}] Training set time range: {self.timestamps.min()} to {self.timestamps.max()}"
            )
        elif self.data_type == "val":
            self.data = self.data.iloc[train_end:val_end].copy()
            self.timestamps = self.timestamps.iloc[train_end:val_end].copy()
            print(
                f"[{self.data_type.upper()}] Validation set: time points {train_end + 1} to {val_end} ({self.val_ratio})"
            )
            print(
                f"[{self.data_type.upper()}] Validation set time range: {self.timestamps.min()} to {self.timestamps.max()}"
            )
            ...
            print(
                f"[{self.data_type.upper()}] Test set time range: {self.timestamps.min()} to {self.timestamps.max()}"
            )
...
def _build_dataset(config, data_type: str):
    path = config.data_path
    if os.path.isdir(path):
        return MultiSymbolKlineDataset(
            data_path=path,
            data_type=data_type,
            ...
            min_bars=getattr(config, "min_bars", 450),
        )
...
def create_dataloaders(config):
    ...
    val_dataset = _build_dataset(config, "val")

    use_ddp = dist.is_available() and dist.is_initialized()
    train_sampler = (
        DistributedSampler(train_dataset, num_replicas=dist.get_world_size(), rank=dist.get_rank(), shuffle=True)
        if use_ddp
        else None
    )
    val_sampler = (
        DistributedSampler(
            val_dataset, num_replicas=dist.get_world_size(), rank=dist.get_rank(), shuffle=False, drop_last=False
        )
        if use_ddp
        else None
    )
    ...
    for epoch in range(config.basemodel_epochs):
        ...
        for batch_idx, (batch_x, batch_x_stamp) in enumerate(train_loader):
            ...
            if (batch_idx_global + 1) % config.log_interval == 0:
                ...
                log_msg = (
                    f"[Epoch {epoch + 1}/{config.basemodel_epochs}, Step {batch_idx + 1}/{len(train_loader)}] "
        ...
        if use_ddp:
            tensor_sum = torch.tensor(
                [epoch_train_loss, train_batches, val_loss, val_batches], dtype=torch.float64, device=device
            )
...
def main():
    ...
    if getattr(config, "pre_trained_tokenizer", True):
        ...
        cfg_path_tok = os.path.join(
            config.pretrained_tokenizer_path
            if hasattr(config, "pretrained_tokenizer_path")
            else config.finetuned_tokenizer_path,
            "config.json",
        )
